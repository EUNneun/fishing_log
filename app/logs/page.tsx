...
export default function LogsPage() {
  ...
  return <Shell>
    ...
    <section className="px-4 pb-28 pt-2">
      {loading?<Empty text="기록을 불러오는 중..."/>:logs.length===0?<Empty text="첫 출조를 기록해보세요."/>:<div className="space-y-3">{logs.map(log=><article key={log.id} className="rounded-[20px] border border-[#e3e7f0] bg-white p-4">
        ...
        {log.memo?<p className="mt-3 border-t border-[#eef0f5] pt-3 text-sm leading-6 text-[#7e8495]">{log.memo}</p>:null}
      </article>)}</div>}
    </section>

...
export default function LogsPage() {
  ...
  useEffect(()=>onAuthStateChanged(auth, async next=>{ if(!next){setLogs(getGuestLogs<Log>() .sort((a,b)=>b.tripDate.localeCompare(a.tripDate)));setLoading(false);return;} try{const snap=await getDocs(collection(db,"users",next.uid,"logs"));setLogs(snap.docs.map(d=>({id:d.id,...d.data()} as Log)).sort((a,b)=>b.tripDate.localeCompare(a.tripDate)));}finally{setLoading(false);}}),[]);
  ...
  return <Shell>
    ...
    <section className="px-4 pb-28 pt-2">
      {loading?<Empty text="기록을 불러오는 중..."/>:logs.length===0?<Empty text="첫 출조를 기록해보세요."/>:<div className="space-y-3">{logs.map(log=><article key={log.id} onClick={()=>{window.location.href=`/log-detail/?id=${encodeURIComponent(log.id)}`}} role="button" tabIndex={0} className="cursor-pointer rounded-[20px] border border-[#e3e7f0] bg-white p-4 transition active:scale-[.99]">
        <div className="pr-10"><div className="flex items-center gap-1.5 text-xs text-[#8a90a0]"><CalendarDays className="size-3.5"/>{log.tripDate}</div><div className="mt-2 flex items-baseline gap-2"><h2 className="text-lg font-extrabold text-[#2f3142]">{log.species}</h2>{log.tripDate>todayKey()?<strong className="rounded-full bg-[#e8f3ff] px-2 py-1 text-xs text-[#3988f2]">출조 예정</strong>:<><strong className="text-lg text-[#3988f2]">{log.catchCount}마리</strong>{log.maxSize?<span className="text-xs text-[#7e8495]">최대 {log.maxSize}cm</span>:null}</>}</div></div>
        <div className="mt-4 grid grid-cols-2 gap-x-3 gap-y-2 text-[13px] text-[#687086]"><span className="flex min-w-0 items-center gap-1.5"><MapPin className="size-3.5 shrink-0 text-[#74a9e8]"/><span className="truncate">{log.location}</span></span><span className="flex min-w-0 items-center gap-1.5"><ShipWheel className="size-3.5 shrink-0 text-[#74a9e8]"/><span className="truncate">{log.boatName}</span></span><span className="flex items-center gap-1.5"><WalletCards className="size-3.5 text-[#74a9e8]"/>{money.format(log.fee||0)}원</span><span className="flex min-w-0 items-center gap-1.5"><Waves className="size-3.5 shrink-0 text-[#74a9e8]"/><span className="truncate">{log.tripDate>todayKey()?(log.rig||"일정 등록"):`${log.weather}${log.rig?` · ${log.rig}`:""}`}</span></span></div>
        {log.memo?<p className="mt-3 border-t border-[#eef0f5] pt-3 text-sm leading-6 text-[#7e8495]">{log.memo}</p>:null}<div className="mt-3 flex items-center justify-between gap-2 border-t border-[#eef0f5] pt-3"><TripPointLabel tripId={log.id} {...points} /><span className="text-right text-xs font-bold text-[#3988f2]">상세 · HIT 기록 보기 ›</span></div>
      </article>)}</div>}
    </section>
    <a href="/record/" aria-label="출조 기록 추가" className="fixed bottom-24 right-[max(22px,calc((100vw-430px)/2+22px))] z-40 grid size-14 place-items-center rounded-full bg-[#3988f2] text-white shadow-lg shadow-[#3988f2]/25"><Plus className="size-6"/></a>
  </Shell>;
}

...
export default function ReportPage(){
 const [logs,setLogs]=useState<Log[]>([]),[loading,setLoading]=useState(true);
 useEffect(()=>onAuthStateChanged(auth,async next=>{if(!next){setLogs(getGuestLogs<Log>());setLoading(false);return}try{const s=await getDocs(collection(db,"users",next.uid,"logs"));setLogs(s.docs.map(d=>({id:d.id,...d.data()} as Log)))}finally{setLoading(false)}}),[]);
 ...
 return <Shell>
  ...
  <section className="space-y-5 px-4 pb-28 pt-2">
   {loading?<Empty text="리포트를 만드는 중..."/>:logs.length===0?<Empty text="출조 기록이 쌓이면 나만의 낚시 패턴을 분석해드려요."/>:<>
    ...
    <Card title="어종별 조과" sub="누적 마릿수"><div className="mt-3 space-y-3">{report.species.slice().sort((a,b)=>b.count-a.count).slice(0,5).map(s=>{const max=Math.max(1,...report.species.map(x=>x.count));return <div key={s.name}><div className="mb-1 flex justify-between text-xs"><b className="text-[#4d5364]">{s.name}</b><span className="text-[#7e8495]">{s.count}마리</span></div><div className="h-2 overflow-hidden rounded-full bg-[#edf0f5]"><div className="h-full rounded-full bg-[#6eacf7]" style={{width:`${s.count/max*100}%`}}/></div></div>})}</div></Card>

...
const tideNames = ["1물", "2물", "3물", "4물", "5물", "6물", "7물", "8물", "9물", "10물", "11물", "12물", "13물", "조금", "무시"] as const;
...
const freshForm = () => ({
  tripDate: today(), location: "", boatName: "", fee: "", species: "", rig: "", weather: "맑음",
  tide: "", highTide: "", lowTide: "", catchCount: "", maxSize: "", memo: "",
  boatCondition: "", captainSkill: "", mealRating: "",
});
...
export default function RecordPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault(); if (!user) return; setSaving(true); setError("");
    try {
      await addDoc(collection(db, "users", user.uid, "logs"), {
        tripDate: form.tripDate, location: form.location.trim(), boatName: form.boatName.trim(), fee: Number(form.fee),
        species: form.species, rig: form.rig.trim(), weather: form.weather, tide: form.tide,
        highTide: form.highTide || null, lowTide: form.lowTide || null,
        catchCount: Number(form.catchCount), maxSize: form.maxSize ? Number(form.maxSize) : null, memo: form.memo.trim(),
        boatCondition: form.boatCondition ? Number(form.boatCondition) : null, captainSkill: form.captainSkill ? Number(form.captainSkill) : null,
        mealRating: form.mealRating ? Number(form.mealRating) : null, createdAt: serverTimestamp(),
      });
    ...
    <main className="min-h-dvh bg-[#eaf3ff] text-[#29456f]">
      <div className="mx-auto min-h-dvh max-w-md bg-[#f7fbff] pb-10 shadow-2xl shadow-[#4a8ee8]/15 md:my-6 md:min-h-[calc(100dvh-3rem)] md:rounded-[2rem]">
        <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-[#dbe9fa] bg-[#f7fbff]/95 px-5 pb-4 pt-[max(1rem,env(safe-area-inset-top))] backdrop-blur">
          ...
          <div><h1 className="text-xl font-extrabold text-[#29456f]">출조 기록</h1><p className="mt-0.5 text-xs text-[#8aa0be]">필수 항목만 입력해도 저장할 수 있어요.</p></div>
        </header>
        ...
        <form onSubmit={submit} className="space-y-5 px-5 py-5">
          ...
          <section className="space-y-4 rounded-[1.5rem] border border-[#dfeafa] bg-white p-4 shadow-sm">
            <div className="grid grid-cols-2 gap-3">
              <Field label="출조일"><Input type="date" required value={form.tripDate} onChange={set("tripDate")} className="min-w-0 w-full" /></Field>
              <Field label="날씨"><NativeSelect required value={form.weather} onChange={set("weather")} className="w-full min-w-0"><NativeSelectOption>맑음</NativeSelectOption><NativeSelectOption>흐림</NativeSelectOption><NativeSelectOption>비</NativeSelectOption><NativeSelectOption>바람</NativeSelectOption><NativeSelectOption>눈</NativeSelectOption></NativeSelect></Field>
            </div>
            <div className="grid grid-cols-2 gap-3"><Field label="장소(항구)"><Input required placeholder="예: 오천항" value={form.location} onChange={set("location")} /></Field><Field label="배 이름(선사)"><Input required placeholder="예: 뉴○○호" value={form.boatName} onChange={set("boatName")} /></Field></div>

            <div className="rounded-2xl bg-[#f5f9ff] p-3">
              <div className="mb-3"><p className="text-sm font-extrabold text-[#496789]">물때</p><p className="mt-1 text-xs text-[#8aa0be]">선택한 항구와 출조일 기준으로 기록해 두세요.</p></div>
              <div className="grid grid-cols-3 gap-2">
                <Field label="물때"><NativeSelect value={form.tide} onChange={set("tide")} className="w-full min-w-0"><NativeSelectOption value="">선택</NativeSelectOption>{tideNames.map((name) => <NativeSelectOption key={name} value={name}>{name}</NativeSelectOption>)}</NativeSelect></Field>
                <Field label="만조"><Input type="time" value={form.highTide} onChange={set("highTide")} className="min-w-0 w-full px-2" /></Field>
                <Field label="간조"><Input type="time" value={form.lowTide} onChange={set("lowTide")} className="min-w-0 w-full px-2" /></Field>
              </div>
            </div>

            <Field label="선비"><Input type="number" min="0" required inputMode="numeric" placeholder="원 단위" value={form.fee} onChange={set("fee")} /></Field>
            <div className="grid grid-cols-2 gap-3"><Field label="어종"><NativeSelect required value={form.species} onChange={set("species")} className="w-full"><NativeSelectOption value="">선택</NativeSelectOption>{speciesNames.map((name) => <NativeSelectOption key={name} value={name}>{name}</NativeSelectOption>)}</NativeSelect></Field><Field label="채비 (선택)"><Input placeholder="예: 가지채비" value={form.rig} onChange={set("rig")} /></Field></div>
          ...
          <Button disabled={saving} type="submit" className="h-13 w-full rounded-2xl bg-gradient-to-r from-[#5e9bf2] to-[#61c5f3] font-extrabold text-white shadow-lg shadow-[#4d94e8]/20 hover:from-[#4f8ee8] hover:to-[#4db7ea]">{saving ? "저장 중..." : "기록 저장"}</Button>
        </form>
      </div>
    </main>
  );
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <div className="min-w-0 space-y-2"><Label className="text-sm text-[#496789]">{label}</Label>{children}</div>; }
function StarRating({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <div className="flex items-center justify-between gap-3"><span className="text-sm font-medium text-[#607a9e]">{label}</span><div className="flex items-center gap-1" role="radiogroup" aria-label={`${label} 별점`}>{[1,2,3,4,5].map((score) => <button key={score} type="button" role="radio" aria-checked={value === score} aria-label={`${score}점`} onClick={() => onChange(score)} className={`rounded-md p-0.5 transition ${score <= value ? "text-[#f5b83d]" : "text-[#c8d7e8]"}`}><Star className="size-7" fill={score <= value ? "currentColor" : "none"} /></button>)}</div></div>; }
